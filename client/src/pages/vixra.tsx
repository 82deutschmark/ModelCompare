...
export default function VixraPage() {
  ...
  const generateSectionMutation = useMutation({
    mutationFn: async (data: { sectionId: string; template: string; variables: Record<string, string>; modelId: string }) => {
      console.debug('[Vixra] mutationFn invoked', { sectionId: data.sectionId, modelId: data.modelId });
      // Build seat using the provided modelId. If model metadata hasn't loaded yet,
      // fall back to reasonable placeholders so we still send the request.
      const modelMeta = models.find(m => m.id === data.modelId);
      const seat: ModelSeat = {
        id: data.sectionId,
        model: {
          id: data.modelId,
          name: modelMeta?.name || data.modelId,
      ...
      console.debug('[Vixra] POST /api/generate payload', { seat, hasTemplate: !!data.template, varKeys: Object.keys(data.variables) });
    ...
    onSuccess: (data) => {
      console.debug('[Vixra] mutation success', { sectionId: data.sectionId, modelId: data.modelId });
      if (!workflowState) return;
    ...
    onError: (error, variables) => {
      console.error('[Vixra] mutation error', error);
      setWorkflowState(prev => {
        if (!prev) return prev;
  ...
  const continueWorkflow = () => {
    ...
    // Wait until models are loaded to ensure seat metadata is available
    ...
    if (nextSections.length === 0) {
      // This can happen if the workflow is complete but the check at the start didn't catch it,
      // or if there's a dependency issue. Stop the workflow and notify the user.
      console.debug('[Vixra] continueWorkflow: No eligible sections found to process.');
      setIsWorkflowRunning(false);
      // Avoid showing a toast if the workflow just completed successfully.
      if (!isWorkflowComplete(workflowState)) {
        toast({
          title: "Workflow Stalled",
          description: "No further sections could be generated. Check dependencies.",
          variant: "destructive",
        });
      }
      return;
    ...
    if (!template) {
      console.warn('[Vixra] Missing template for section', nextSectionId);
      return;
    }
    ...
    // Generate section
    console.debug('[Vixra] triggering generate for section', { nextSectionId, modelId });
  ...
  const startWorkflow = () => {
    console.debug('[Vixra] startWorkflow clicked', { hasWorkflowState: !!workflowState, selectedModelsCount: selectedModels.length, modelsLoading });
    ...
    // Start processing
    setTimeout(() => {
      console.debug('[Vixra] continueWorkflow scheduled');
      continueWorkflow();
    }, 100);
  };
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      ...
                      disabled={isWorkflowRunning || selectedModels.length === 0 || !workflowState || modelsLoading}
