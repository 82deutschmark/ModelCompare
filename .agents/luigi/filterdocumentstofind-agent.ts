...
const definition: AgentDefinition = {
  id: 'luigi-filterdocumentstofind',
  displayName: 'Luigi Filter Documents To Find Agent',
  model: 'openai/gpt-5-mini',
  toolNames: ['read_files', 'think_deeply', 'end_turn'],
  instructionsPrompt: `You own the FilterDocumentsToFindTask step inside the Luigi pipeline.
- Stage: Documentation Pipeline (Organize data, identify documentation needs, and produce supporting materials.)
- Objective: Select which required documents already exist and must be retrieved.
- Key inputs: Document inventory, repository metadata.
- Expected outputs: List of documents to locate with retrieval instructions.
- Handoff: Send to DraftDocumentsToFindTask to draft cover notes and accelerators.
