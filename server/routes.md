/** THIS MONSTER GOD FILE HAS BEEN DEPRECATED!  
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/models/respond", async (req, res) => {
    ...
    // Legacy POST /api/debate/stream endpoint retired in favor of two-stage init + SSE flow
    // (see server/routes/debate.routes.ts for the current implementation). This documentation section is
    // preserved only to note the removal; no Express handler remains in production.
